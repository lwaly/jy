...
bool CmdExecSql::AnyMessage(std::shared_ptr<neb::SocketChannel> pChannel, const MsgHead& oMsgHead, const MsgBody& oMsgBody)
{
    ...
    if (!oMemOperate.ParseFromString(oMsgBody.data()))
    {
        ...
        Response(neb::ERR_PARASE_PROTOBUF, "neb::Mydis ParseFromString() failed!");
        return(false);
    }
                ...
                CMysqlDbi** ppMasterDbi, CMysqlDbi** ppSlaveDbi)
{
    ...
    if (c_factor_iter == m_mapFactorSection.end())
    {
        ...
        Response(neb::ERR_LACK_CLUSTER_INFO, "no db config found for oMemOperate.cluster_info()!");
        return(false);
    }
    else
    {
        ...
        if (c_section_iter == c_factor_iter->second.end())
        {
            ...
            Response(neb::ERR_LACK_CLUSTER_INFO, "no db config for the cluster info!");
            return(false);
        }
        else
        {
            ...
            {
                Response(neb::ERR_LACK_CLUSTER_INFO, "no db instance config for db group!");
            ...
            {
                Response(neb::ERR_NO_RIGHT, "no right to excute oMemOperate.db_operate().query_type()!");
                ...
                const neb::CJsonObject& oInstanceConf, CMysqlDbi** ppMasterDbi, CMysqlDbi** ppSlaveDbi)
{
    ...
    *ppSlaveDbi = NULL;
    ...
    if (dbi_iter == m_mapDbiPool.end())
    {
        ...
        if (NULL == pConnection)
        {
            Response(neb::ERR_NEW, "malloc space for db connection failed!");
            return(false);
        }
...
int CmdExecSql::Query(const neb::Mydis& oQuery, CMysqlDbi* pDbi)
{
    ...
    if (NULL == pDbi)
    {
        ...
        Response(neb::ERR_QUERY, "pDbi is null");
        return(neb::ERR_QUERY);
    }
        ...
        if (neb::Mydis::DbOperate::SELECT == oQuery.db_operate().query_type())
        {
            if (NULL != pDbi->UseResult())
            {
                ...
                while (NULL != (stRow = pDbi->GetRow()))
                {
                    ...
                    {
                        LOG4_ERROR("error %d: %s", neb::ERR_RESULTSET_EXCEED, "result set(one record) exceed 1 MB!");
                        Response(neb::ERR_RESULTSET_EXCEED, "result set(one record) exceed 1 MB!");
                        return(neb::ERR_RESULTSET_EXCEED);
                    }
